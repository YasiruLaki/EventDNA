...
<head>
  ...
  <title>Update Group - EventDNA</title>
...
<body>
  ...
  <div class="dashboard-shell">
    <main class="dashboard-content">
      ...
      <div id="createGroupView">
        ...
        <h1 class="page-title" style="margin-bottom:0.25rem;">Update Group</h1>
        <p class="supporting-copy" style="margin-bottom:2rem;">Modify your community group details.</p>
      ...
      <article id="successView" class="stat-card success-card" style="padding: 2.5rem;">
        <h2 style="color: var(--secondary);">Group Updated Successfully</h2>
        <p style="color: var(--text-secondary);">Your changes have been saved.</p>
  ...
  <script>
    ...
    // First load interests, then load group details
    fetch('../groups/interests-list.php')
      ...
      .then(result => {
        ...
        result.data.forEach(interest => {
          ...
          pill.dataset.id = interest.interest_id;
        ...
        // Now load group details
        return fetch(`../groups/get.php?group_id=${groupId}`);
      })
      ...
      .then(groupResult => {
        ...
        if (data.interests && data.interests.length > 0) {
          data.interests.forEach(interest => {
            selectedInterestIds.add(interest.interest_id);
            const pill = tagPicker.querySelector(`button[data-id="${interest.interest_id}"]`);
            if (pill) {
              pill.classList.add('selected');
            }
          });
      ...
      .catch(() => {
        tagPicker.innerHTML = '<span style="color:var(--text-secondary); font-size:0.85rem;">Could not load data.</span>';
      });
    ...
    form.addEventListener('submit', function (e) {
      ...
      .catch(() => {
        submitBtn.disabled = false;
        submitBtn.textContent = 'Save Changes';
        submitError.textContent = 'The group could not be updated. Please try again.';
        submitError.style.display = 'inline';
      });
    });
  </script>
