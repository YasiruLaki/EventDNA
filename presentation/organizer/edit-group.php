...
if (!$group || (int)$group['creator_id'] !== $organizerId) { die("Unauthorized or group not found."); }

$interests = $groupController->getInterests();
$errors = [];
$selectedInterests = array_column($group['interests'], 'interest_id');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $result = $groupController->updateGroup($groupId, $organizerId, $data);
    if ($result['success']) {
        header("Location: view-group.php?id=" . $groupId);
        exit;
    }
...
<head>
    <meta charset="UTF-8">
    <title>Edit <?= htmlspecialchars($group['name']) ?> - EventDNA</title>
    ...
    <div class="dashboard-shell">
        <main class="dashboard-content">
            <div class="org-hero-header" style="max-width: 800px; margin: 0 auto 2rem;">
                <h1 class="page-title">Edit Community</h1>
                <p class="supporting-copy">Update details for <?= htmlspecialchars($group['name']) ?></p>
            </div>
            ...
            <form method="post" class="form-card">
                <div class="form-group">
                    ...
                    <input type="text" name="name" class="form-input" required value="<?= htmlspecialchars($_POST['name'] ?? $group['name']) ?>">
                </div>
                <div class="form-group">
                    <label>Description</label>
                    <textarea name="description" class="form-textarea" rows="5" required><?= htmlspecialchars($_POST['description'] ?? $group['description']) ?></textarea>
                </div>
                <div class="form-group">
                    ...
                    <div class="tag-list">
                        <?php foreach ($interests as $interest): ?>
                            <label class="tag-chip">
                                <input type="checkbox" name="interests[]" value="<?= $interest['interest_id'] ?>" <?= in_array($interest['interest_id'], $selectedInterests) ? 'checked' : '' ?>>
